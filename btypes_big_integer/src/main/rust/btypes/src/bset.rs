...
impl<T: 'static + BObject> BSet<T> {
    ...
    pub fn interval(a: &BInteger, b: &BInteger) -> BSet<BInteger> {
        ...
        let mut i = a.clone();
        while i.lessEqual(b).booleanValue() {
            result.set.insert(BInteger::new_from_bigint(i.get_val().clone()));
            i.inc()
        }
        return result;
    }
    ...
    pub fn _size(&self) -> BInteger {
        return BInteger::new_from_bigint(self.set.len().to_bigint().unwrap());
    }

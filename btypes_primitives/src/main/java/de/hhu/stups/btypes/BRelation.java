...
public class BRelation<S,T> extends BSet<BTuple<S,T>> {
	...
	public BRelation<T,S> inverse() {
		...
		for(BTuple<S,T> tuple : this) {
			result = result.union(new BRelation<T,S>(new BTuple<>(tuple.projection2(), tuple.projection1())));
		}
		return result;
	}
	...
	public BRelation<S,T> concat(BRelation<S,T> arg) {
		...
		for(BTuple<S,T> tuple : arg) {
			result = result.union(new BRelation<S,T>(new BTuple<>((S) size.plus((BInteger) tuple.projection1()), tuple.projection2())));
		}
		return result;
	}
	...
	public BRelation<S,T> prepend(T arg) {
		BRelation<S,T> result = new BRelation<S,T>(new BTuple<>((S) new BInteger(1), arg));
		for(BTuple<S,T> tuple : this) {
			result = result.union(new BRelation<S,T>(new BTuple<>((S) ((BInteger) tuple.projection1()).succ(), tuple.projection2())));
		}
		return result;
	}
		...
		for(BTuple<S,T> e1 : this) {
			for(BTuple<S,R> e2 : arg) {
				if(e1.projection1().equals(e2.projection1())) {
					result = result.union(new BRelation<S, BTuple<T,R>>(new BTuple<S, BTuple<T,R>>(e1.projection1(), new BTuple<T,R>(e1.projection2(), e2.projection2()))));
				}
			}
		}
		return result;
	...
	public static <T> BRelation<T,T> identity(BSet<T> arg) {
		...
		for(T e : arg) {
			result = result.union(new BRelation<T,T>(new BTuple<>(e,e)));
		}
		return result;
	}
